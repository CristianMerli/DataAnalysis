...
def cln_invalid_data(x_arr, y_arr):                                                                                     # cln_invalid_data(X-array, Y-array)
  x_cln_arr = list()                                                                                                    # Define empty cleaned X-array list
  y_cln_arr = list()                                                                                                    # Define empty cleaned Y-array list
...
# Function definition to apply selected polynomial approximation (interpolation/curve-fitting)
...
def poly_approx(x_arr, y_arr, approx_mode, typ_ord):                                                                    # poly_approx(X-array, Y-array, Approx-mode: poly-interpolation/poly-fitting, Poly-interpolation-type/Fitting-poly-order)
  x_cln_arr, y_cln_arr = cln_invalid_data(x_arr, y_arr)                                                                 # Function call to get X and Y cleaned arrays deletin' invalid vals
  if (approx_mode == Poly_approx_md.int):                                                                               # In case of poly-interpolation approx mode sel
    return interp1d(x_cln_arr, y_cln_arr, kind=typ_ord), x_cln_arr, y_cln_arr                                           # Return polynomial interpolation function and X/Y cleaned arrays
  else:                                                                                                                 # Else in case of poly-fitting approx mode sel
    return poly.Polynomial.fit(x_cln_arr, y_cln_arr, typ_ord), x_cln_arr, y_cln_arr                                     # Return polynomial fitting function and X/Y cleaned arrays
